...
class Position:
    ...
    def nb_moves(self) -> int:
        ...
        for i in range(HEIGHT):
            count += reduce(lambda x, y: x + y, [1 if j != -1 else 0 for j in self.state.get_tensors()[0][i]])
        return count
    ...
    def to_bitboards(self) -> (int, int):
        player1 = int('0b' + ''.join([str(1 if j == 0 else 0) for i in self.state.get_tensors()[0] for j in i]), 2)
        player2 = int('0b' + ''.join([str(1 if j == 1 else 0) for i in self.state.get_tensors()[0] for j in i]), 2)
...
class Solver:
    def __init__(self):     
        ...
        self.column_order = [3, 4, 2, 5, 1, 6, 0]  # For WIDTH=7, adjust accordingly for other sizes
    ...
    def negamax(self, position, alpha=-HEIGHT * WIDTH, beta=HEIGHT * WIDTH) :
        ...
        if position.to_bitboards() in self.memo:  # Look up the position in the transposition table
            return self.memo[position.to_bitboards()]

        best_move = -1
        max_score = (WIDTH * HEIGHT - 1 - position.nb_moves()) // 2  # Upper bound of our score
        if beta > max_score:
            beta = max_score  # Adjust beta if necessary
            if alpha >= beta:
                return beta, -1 # Prune the exploration if the alpha-beta window is empty

        for x in self.column_order:  # Compute the score of all possible next moves and keep the best one
            if position.can_play(x):
                ...
                temp_score, _ = self.negamax(position_copy, -beta, -alpha)  # Negate the score and reverse the alpha-beta window
                score = -temp_score

                if score >= beta:
                    self.memo[position.to_bitboards()] = beta, x
                    return score,x  # Prune the exploration if we find a move better than what we were looking for
                if score > alpha:
                    best_move = x
